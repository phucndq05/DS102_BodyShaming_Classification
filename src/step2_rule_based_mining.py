...
def get_potential_teencode(data_path, text_col='comment', output_path='teencode_candidates.csv', top_n=200, preprocessor=None):
    """
    Hàm thống kê và phát hiện các từ có khả năng là teencode từ dữ liệu.
    preprocessor (DataPreprocessor, optional): Nếu đã khởi tạo sẵn, truyền vào để dùng lại.
    Args:
...
if __name__ == "__main__":
    ...
    DATA_PATH = 'data/raw/dataset_filtered.csv'
    OUTPUT_PATH = 'data/dictionaries/teencode_candidates.csv'
    TEEN_PATH = 'data/dictionaries/teencode.csv'  # Thêm đường dẫn teencode

    # Khởi tạo preprocessor với từ điển teencode
    preprocessor = DataPreprocessor(teencode_path=TEEN_PATH)

    # Kiểm tra file tồn tại trước khi chạy
    if os.path.exists(DATA_PATH):
        get_potential_teencode(DATA_PATH, text_col='comment_text', output_path=OUTPUT_PATH, preprocessor=preprocessor)
    else:
